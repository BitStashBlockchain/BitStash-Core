...
ResultExecute BitStashState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BitStashTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, BitStashTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, BitStashTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}
...
void BitStashState::validateTransfersWithChangeLog(){
...
void BitStashState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!BitStashState::addressInUse(delegationsAddress)){
        BitStashState::createContract(delegationsAddress);
        BitStashState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
