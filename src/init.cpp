 ...
 */
static const char* BITCOIN_PID_FILENAME = "bitstashd.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("bitstash-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitstashproject/bitstash>";
    const std::string URL_WEBSITE = "<https://bitstash.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateBitStash");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                globalSealEngine->setBitStashSchedule(bitstashDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
