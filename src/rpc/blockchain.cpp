...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // bitstash
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // bitstash
    }
...
UniValue transactionReceiptToJSON(const BitStashTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        ...
        if (IsValidDestination(bitstashSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&bitstashSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitstash address string"},
                },
                RPCResult{
            "{\n"
    ...
    if(!bitstashDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = bitstashDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitstash address string for staker"},
                },
                RPCResult{
            "[{\n"
    ...
    if(!bitstashDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = bitstashDelegation.DelegationsFromEvents(events);
