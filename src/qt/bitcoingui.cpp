...
void BitcoinGUI::createActions()
{
    ...
    BRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&BRC Tokens"), this);
    BRCTokenAction->setStatusTip(tr("BRC Tokens (send, receive or add Tokens in list)"));
    BRCTokenAction->setToolTip(BRCTokenAction->statusTip());
    BRCTokenAction->setCheckable(true);
    BRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(BRCTokenAction);
    ...
    connect(BRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(BRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    openAction->setStatusTip(tr("Open a bitstash: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitStash command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(BRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        {
            QString link = QString("<a href=%1>%2</a>").arg(BITSTASH_RELEASES, BITSTASH_RELEASES);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    BRCTokenAction->setEnabled(enabled);
...
{
    BRCTokenAction->setChecked(true);
