...
BitStashDelegation& GetBitStashDelegation()
{
    static BitStashDelegation bitstashDelegation;
    return bitstashDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, CValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        BitStashDelegation& bitstashDelegation = GetBitStashDelegation();
        if(!bitstashDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = bitstashDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    BitStashDelegation& bitstashDelegation = GetBitStashDelegation();
    bool ret = bitstashDelegation.GetDelegation(address, delegation);
    if(ret) ret &= bitstashDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
