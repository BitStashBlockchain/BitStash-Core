...
//    initState();
    ...
    txs.push_back(createBitStashTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createBitStashTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createBitStashAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BitStashTransaction> txsCall;
    txsCall.push_back(createBitStashTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<BitStashTransaction> txsMint;
    txsMint.push_back(createBitStashTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<BitStashTransaction> txsbalance;
    txsbalance.push_back(createBitStashTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<BitStashTransaction> txs;
    txs.push_back(createBitStashTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createBitStashAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BitStashTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createBitStashTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
